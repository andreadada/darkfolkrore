...
This document describes Dark Folklore Core 0.1.0 for Minecraft 1.21.1, NeoForge 21.1.248, and Java 21. It records the installed versions that were audited, not a promise that later releases of those mods are compatible.
...
| werewolves | 2.0.3.3 | Vampirism public faction registry plus native tags | Full factual werewolf detection when the Vampirism adapter is active; native silver semantics are preserved; the Core's duplicate Fangs natural-spawn suppression follows the canonicalization toggle. |
| mca | 7.7.32+1.21.1 | Standard entity events and MCA namespace filtering | Core does not alter MCA state. Supernatural facts come from the exact MCA Vamp Compat bridge below. |
...
| fieldguide | 1.14.0 | Public unlock type plus public server progress implementation | Kill unlock forwarding, bounded unlock-to-lore discovery polling, 11 tag-driven categories, and five static descriptions. Tiered dynamic descriptions are not implemented. |
| enchanted | 4.2.7 | Registry IDs, optional tags, canonical loot routing | Garlic and wolfsbane semantic tags; while canonicalization is enabled, Enchanted garlic generated through loot is routed to Vampirism garlic. Mandrake, altars, and poppets remain distinct. No Enchanted Java internals are called. |
...
| quest_giver | 1.5.1 | Detection only | Optional future/front-end integration. Dark Folklore owns contract state; no Quest Giver bridge is implemented in 0.1.0. |
...
The manager also reports Enchanted, Occultism, Malum, Eidolon, Feywild, Better Archeology, Quest Giver, and AlmostUnified as ACTIVE when their exact versions are present. For data-only or detection-only rows, ACTIVE means “exact audited pack component found,” not “a private Java adapter is running.”
...
Vampirism and Field Guide adapters are loaded by class name only after their exact versions are confirmed. Their external types therefore do not enter common initialization when the mods are absent. MCA Vamp Compat reflection is isolated in McaVampCompatAdapter and validates every expected method before it is published to the state-adapter list.
